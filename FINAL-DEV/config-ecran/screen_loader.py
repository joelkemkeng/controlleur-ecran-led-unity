...
class ScreenConfigLoader:
    ...
    def __init__(self, excel_file: str = None):
        # Par défaut, chercher le fichier depuis la racine du projet
        ...
        for index, row in df.iterrows():
            try:
                ...
                for entity_id in range(entity_start, entity_end + 1):
                    # Calculer le canal DMX (relatif dans l'univers)
                    channel = (entity_id - entity_start) + 1
                ...
                if index < 3:
                    print(f"🗺️  [ScreenConfig] Ligne {index}: {entity_start}-{entity_end} → {controller_ip}:u{universe}")
